...
dirtyCallback=null;
	...
	var eyeOff = function(cId) {
        ...
        function handlePrintMessage_(message, pausetime)
        {
                showLetter(message[0]);
                var rest = message.substr(1);
                if (rest.length) {
                        setTimeout(
                            function() { handlePrintMessage_(rest, pausetime);},
                            pausetime);
                } else {
	...
	var printMessage = function(message, pausetime) {
		...
		handlePrintMessage_(message,printMessageInterval);
	};
...
//    void showViewport(Image& someImage, int x, int y) {
	...
	StringSprite.prototype.pixel_width = function() {
        ...
        function handleScrollSprite_(sprite, scrollOffsetH, pausetime)
        {
            if (scrollOffsetH< sprite.pixel_width())
            {
                sprite.render_string();
                sprite.pan_right();
                var newScrollOffsetH = scrollOffsetH+1;
                setTimeout(
                    function() {
                        handleScrollSprite_(sprite, newScrollOffsetH, pausetime)
                    }, pausetime);
            } else {
                micro_device_ready = true;
            }
	...
	var scrollSprite = function(theSprite, delay)
	{
		...
		handleScrollSprite_(spriteToScroll, spriteScrollOffsetH, spriteScrollInterval);
	};
